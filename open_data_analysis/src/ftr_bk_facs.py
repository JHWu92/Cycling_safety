...
def get_feature_bk_facs(path_osm_db, path_segs, path_ftr_bk_facs, bfr_crs, bfr_func, init_crs=4326, debug=False):
    ...
    # get seg-wise features by groupby
    features = []
    for idx, grp in matched.groupby(index_seg):
        features.append(tuple([idx]+[tuple(x for x in grp[c].values if (not pd.isnull(x)) and not x=='') for c in ftr_name_bk_facs]))

    features = pd.DataFrame(features,columns=[index_seg]+ftr_name_bk_facs)
    # refine seg-wise features
